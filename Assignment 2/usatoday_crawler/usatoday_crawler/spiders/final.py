...
class UsaTodayCrawler(CrawlSpider):
    ...
    custom_settings = {
        ...
        'DEPTH_LIMIT': 16,  # Set the depth limit to 16
    }
    ...
    def parse_page(self, response):
        ...
        if content_type in self.allowed_file_types:
            ...
            file_size_kb = len(response.body) / 1024
            
            if content_type == 'text/html':
                outlinks = response.css('a::attr(href), img::attr(src)').extract()
                num_outlinks = len(outlinks)
                self.visit_writer.writerow([response.url, f"{file_size_kb:.2f} KB", num_outlinks, content_type])
                
                for outlink in outlinks:
                    parsed_outlink = self.process_outlink(outlink)
                    if parsed_outlink:
                        yield response.follow(parsed_outlink, callback=self.parse_page, errback=self.errback)
            else:  # For image types
                self.visit_writer.writerow([response.url, f"{file_size_kb:.2f} KB", 0, content_type])
    ...
    def errback(self, failure):
        ...
        # Check if it's a failure with a response (which would have the status code)
        if failure.check(HttpError):
            response = failure.value.response
            status = response.status  # Get the actual HTTP status code
            self.fetch_writer.writerow([url, status])  # Log the actual status code
            logging.error(f"Request failed with status: {status} for URL: {url}")
        else:
            # Handle non-HTTP errors
            self.fetch_writer.writerow([url, 'Unknown Error'])
            logging.error(f"Request failed: {url}")
    ...
    def closed(self, reason):
        ...
        self.crawl_report.write(f"Name: Shaswat Srivastava\n")
        self.crawl_report.write(f"USC ID: 4591938879\n")
